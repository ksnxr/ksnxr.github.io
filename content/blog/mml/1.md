...
1. Associativity: $\forall a,b\in G:\quad (a+b)+c=a+(b+c)$
2. Identity element: $\exists e\in F,\;\forall g\in F:\quad e+g=g+e=g$
3. Inverse element: $\forall a\in F,\;\exists b\in F:\quad a+b=b+a=e$
...
* $\newcommand{\coloneqqf}{\mathrel{\vcenter{:}}=}S_{n}\coloneqqf\{\pi:\{a,\dots,n\}\rightarrow\{a,\dots,n\}\vline\pi\; is\; bijective\},\quad\cdot: S_{n}\times S_{n}\rightarrow S_{n},\quad\pi_{1}\cdot\pi_{2}=\pi_{1}(\pi_{2}(i)),\quad(S_{n},\;\cdot)\; is\; a\; group.$
...
* $(R,\; +,\;\cdot)$
* $(C,\; +,\;\cdot)$
* $n\in Z,$ consider $Z_{n}\coloneqqf\{0,1,\dots ,n-1\}$. $a+_{n}b\coloneqqf (a+b)\; mod\; n,$ $a\cdot_{n}b\coloneqqf (a\cdot b)\; mod\; n$. Then $(Z_{n}, +_{n}, \cdot_{n})$ is a field if and only if $n$ is prime.
...
* Function spaces
  ...
  * $R^{x}\coloneqqf\{f:X\rightarrow R\}$ the space of all real valued fucntions on a set $X$. Define: $+:R^{x}\times R^{x}\rightarrow R^{x}, (f+g)(x)\coloneqqf f(x)+g(x)\\\cdot:R\times R^{x}\rightarrow R^{x}, (\lambda\cdot f)(x)=\lambda\cdot (f(x))$,

    Then $(R^{x},+,\cdot)$ is a real vector space.

  * $C(x)\coloneqqf \{f:X\rightarrow R\vert f\; is\; continuous\}$
